<div class="row">
    <div class="col-sm-12">
        ...
        <div class="element-wrapper">
            ...
            <?php 
                $retailer_account = "SELECT id FROM `emp_login` WHERE user_role = 'employee' or user_role = 'admin'";
                $Total_emp = 0;
                if ($result = mysqli_query($connection, $retailer_account)) {
                    $Total_emp = mysqli_num_rows($result);                   
                }

                $retailer_account = "SELECT id FROM emp_login where (user_role='employee' or user_role='admin') and status='1' ";
                $Active_emp = 0;
                if ($result = mysqli_query($connection, $retailer_account)) {
                ...
                $retailer_account = "SELECT t2.task_id FROM `assign_task` as t2 JOIN emp_login as t1 ON t1.id = t2.emp_id order by work_assign_date desc;";
                $Total_task = 0;
                if ($result = mysqli_query($connection, $retailer_account)) {
                ...
                $retailer_account = "SELECT t1.task_id, t2.emp_name FROM assign_task as t1 JOIN emp_login as t2 ON t2.id = t1.emp_id where t1.emp_id='$emp_id' and t1.status='Close' order by t1.work_assign_date desc";
                
                //SELECT task_id FROM assign_task where status='Close'";
                $close_task = 0;
                if ($result = mysqli_query($connection, $retailer_account)) {
        ...
        <div class="element-wrapper">
            ...
            <div class="element-box-tp">
                <div class="el-buttons-list full-width">
                    ...
                    <a class="btn btn-white btn-sm" href="./employee.php?source=add_emp">
                        <i class="os-icon os-icon-delivery-box-2"></i><span>Add New Employee</span>
                    </a>
                    <a class="btn btn-white btn-sm" href="./create_project.php">
                        <i class="os-icon os-icon-delivery-box-2"></i><span>Create New Project</span>
                    </a>
                    <a class="btn btn-white btn-sm" href="./create_task.php">
